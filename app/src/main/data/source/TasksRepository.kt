...
class TasksRepository(private val tasksRemoteDataSource: TasksRemoteDataSource,
    ...
    override fun getHome(date: String, city: String): Flowable<Result<One>> =
            tasksRemoteDataSource.getHome(date, city)
    ...
    override fun getUser(id: String): Flowable<Result<User>> =
            tasksRemoteDataSource.getUser(id)

    fun saveAccount(vararg account: Account) =
            tasksLocalDataSource.saveAccount(*account)

    fun saveUser(vararg user: User): List<Long> =
            tasksLocalDataSource.saveUser(*user)

    fun getUser(): Flowable<List<User>> =
            tasksLocalDataSource.getUser()

    override fun getSearchWord(type: String, word: String, page: Int): Flowable<Result<Search>> =
            tasksRemoteDataSource.getSearchWord(type, word, page)
}